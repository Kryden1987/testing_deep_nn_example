...
class NeuralNetwork():
    ...
    def get_layer_output(self,X):
        ...
        get_next_layer_output      = tf.keras.backend.function([self.model.layers[0].input],[self.model.layers[1].output])
        get_relu_next_layer_output = tf.keras.backend.function([self.model.layers[0].input],[self.model.layers[2].output])
